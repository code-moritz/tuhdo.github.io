#+TITLE: Emacs Mini Manual (PART 1) - THE BASICS//Emacs迷你手册（第1部分）  - 基礎篇
...
- Please remember that you can always access a section using the
  ...
  screen.請記住，你可以通過屏幕右上角的浮動目录随時訪問本章節的任何部分。

- Many people asked how this manual can be "mini", since it is pretty
  big for an online single page article. 許多人問為什麽這個手册能叫“迷你”，因為它對於一個單頁文章中相當大。Yes, if this page is exported
  to PDF, it is more than 40 pages. 是的，如果這頁文章輸出成為PDF，它將會超過40頁。However, this manual is "mini"
  relative to the size of the official Emacs manual, which is more
  than 600 pages. That's why.但是，這份手册相對官方手册大小是迷你了，官方手册有600頁。這就是為什麽.
...
- If you find the page is loading too slow for you, you can always
  ...
  https://github.com/tuhdo/tuhdo.github.io  如果你發篇目這個頁面加載起來太慢了，你可以克隆一份此迷你書的git 庫離線閱占賣。
...
:END:
In Emacs, you can do many things aside from editing/在Emacs中，你可以通過編輯的方式做很多事情. Emacs is a
programming platform, not just another editor in the sense that people
think/Emacs是一個編程平臺，不僅僅是人們感受到的編輯器. For more details, read the//更多詳情，閱讀 [[*Appendix][Appendix]].

Take a [[http://www.gnu.org/software/emacs/tour/][tour]] from the official Emacs homepage/在官方Emacs主頁上獲取一份快速入門文檔.

Here is my personal tour, *with 3rd party plugins added*  //這是我個人速成手册，*并加入了第三方插件*. Don't worry
if your Emacs does not look like the screenshots//不用担心如果你的Emacs不像屏幕所示的樣子. You can have
something like mine and even better, but for now you need to get a
solid foundation//你能有一些如最小或者更好事情，但是現在你需要獲得更為堅實的基礎. The screenshots are just for show//截屏僅僅是為了展示. Even though,
after going through this guide, you will be able to browse the Linux
kernel source tree with ease//通過這個手册，你將會能輕松瀏覽Linux Kernel代碼樹.

- Performance//性能: Editing a 39MB C source file with no problem//編輯39MB的C代碼文件沒有問題. But well,
  it's not your favorite editor's fault if it crashes from editing
  such a large source file//这是不是你喜欢的编辑器的错，如果从编辑这么大的源文件崩溃.

  [[file:static/performance.gif][file:static/performance.gif]]

- Programming//編碼:
  - Jump to any header file//跳轉到任何頭文件:

    [[file:static/jump_headers.gif][file:static/jump_headers.gif]]

  - Autocompletion//自動補全: Pay attention to completion candidates when
    ~include <linux/printk.h>~ is present and when it's not//注意無論~include <linux/printk.h>~是在呈現，都會有相應補全的候選對像.

    [[file:static/auto_complete.gif][file:static/auto_complete.gif]]

  - Showing function arguments//顯示函數參數:

    [[file:static/func_args.jpg][file:static/func_args.jpg]]

  - Quickly comment multiple lines//快速注釋多行:
...
- Diff between current editing file and latest file revision//當前編輯文件和最後一次編輯版本之間對比:
...
- Quickly select any file in a directory under a Version Control
  System, for example from the Linux kernel//在版本控制係統中的文件夹下快速選擇多個文件,例如Linux kernel. Note that in the demos
  you may see me type in the commands//注意你在演示中，你可能看到我在輸入命令行. You can think of it like the
  start menu in Windows, but those commands can actually be executed
  quickly with a shortcut//你可能想像Windows的開始菜單，但實際上命令的執行更為簡潔.
...
- Quickly select any file/directory from a previous working session//快速選擇多個文件/文件夹從前一個工作事務中:

  [[file:static/helm-buffer-list.gif][file:static/helm-buffer-list.gif]]

- Emacs is a PDF Reader//Emacs是一個PDF閱讀器: I can search text in the PDF file with
  highlighting and a table of contents side by side//我能搜索文本在PDF文件中，并把結果高亮和并排顯示. All can be
  controlled with keyboard//所有都可以通過键盤控制.
...
:END:
Let's look at part of the Emacs manual讓我們來看手册的這部分:
...
files..., many things from basic to advanced//所有處理的文件是在=文件（Files）=入口，此入口包括了如何打開文件，保存文件，獲取和自動保存文件，對比文件....，及許多基礎和高級的功能. If you are a beginner
reading the Emacs manual, you have to actively search the manual to
learn how to do the basic common tasks you know from other editors//如果你是一個初學者閱讀這份Emacs手册，你必須行動起來在手册中找寻和學習如何去做在其他編輯器中日常基本任務.
